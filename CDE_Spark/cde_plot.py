...
def get_label_adafs_spark(series_set, signal_name):
...
def distribution_plot_subsets_spark(time_series_sets, out_dir):
    #data_adafs = subsets
    x_min = -1.0
    x_max = 3.0
    bins = 120
    ...
    logy = False
    signal_name = "OxiCat_facHCCnvRat"
    signal_label = get_label_adafs_spark(time_series_sets, signal_name)

    # The entire signal is needed for distribution fitting so simply
    # extract it all here:
    total_length = 0
    for series in time_series_sets:
        total_length += series.basis().size()
    signal = np.zeros(total_length, dtype=float)
    i = 0
    for series in time_series_sets:

        length = series.basis().size()
        signal[i:i+length] = series.y
        i += length
