...
def main():
    ...
    # vehical_config
    vehical_start_time = time.time()
    vehical_config(adaf_objs)
    vehical_config_finished_time = time.time() - vehical_start_time
    print "%-40s%s" % ("vehical_config_finished_time", vehical_config_finished_time)

    #
    # filter file
    filter_file_start_time = time.time()
    adaf_objs = filter_file(adaf_objs)
    filter_file_finished_time = time.time() - filter_file_start_time
    print "%-40s%s" % ("filter_file_finished_time", filter_file_finished_time)

    # create subsets
    create_subsets_start_time = time.time()
    subsets_list = create_subsets(adaf_objs)
    create_subsets_finished_time = time.time() - create_subsets_start_time
    print "%-40s%s" % ("create_subsets_finished_time", create_subsets_finished_time)
