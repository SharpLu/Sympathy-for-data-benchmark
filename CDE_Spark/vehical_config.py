...
# edit by cocky
def vehical_config_spark(adaf_obj,config):
    """
    update the adaf's meta data from vehical config
    """
    #config = get_config()
    #matched_list = look_up(adaf_objs, config)
    #update_meta(adaf_objs, matched_list)
    rs = look_up_and_update_metadata_spark(adaf_obj,config)
...
def get_config_spark(vehical_config_file_conf):
    # load the config file
    xls_config = pd.ExcelFile(vehical_config_file_conf)
    config = xls_config.parse('Sheet1')
...
def update_meta(adaf_objs, matched_list):
    for i, adaf_obj in enumerate(adaf_objs):
        ...
        for key in matched_config.keys():
            if key != "date":
                adaf_obj.meta.create_column("{}_{}".format("VehicleList", key),np.array([matched_config[key]]))
        ...
        if 'CoEng_st' in adaf_obj.ts.keys():
            #print "adaf_obj.ts.keys ",adaf_obj.ts.keys()
...
def look_up_and_update_metadata_spark(adaf_obj, config):
    """
    look up and update metadata method logic all in this method
    :param adaf_obj:
    :param config:
    :return:
    """
    ...
    adaf_obj.meta.delete_column("Date")
    adaf_obj.meta.delete_column("VIN_Number")
    #if len(matched_list) ==0 or type(matched_list[0]) == type(None):
    #   return

    if found:
        matched_config = matched_list[0]
        for key in matched_config.keys():
            if key != "date":
                adaf_obj.meta.create_column("{}_{}".format("VehicleList", key),np.array([matched_config[key]]))
